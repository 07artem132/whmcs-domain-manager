 ...
 *  Created by PhpStorm.
 *  User: Артём
 *  Date time: 09.02.2020, 10:54
 *
 */

namespace WHMCS\Module\Addon\DomainManager\vendor\SSHClient;
...
class SftpClient
{
     ...
     * @param bool $enableFileSizeVerification
     * @throws Exception
     */
     ...
     * @return $this
     * @throws Exception Invalid connection resource! due to use of validateSshResource.
     * @throws Exception Unable to write to local file
     */
    public function download($remoteFilePath, $localFileName = null)
    {
        ...
        if ($this->fileSizeVerificationEnabled && $fileSize !== filesize($savePath)) {
            throw new Exception("Different file size: " . $savePath);
        }
