<div class="row spacer_10">
	...
	<div class="col-md-3">
		<div class="text-right">
			<!-- Split button -->
			<div class="btn-group">
				<button type="button" class="btn btn-warning btn-sm" onclick="dnssec('rectify', '{$domain->id}');" data-toggle="tooltip" data-placement="bottom" title="{$LANG.admin_manage_dnssec_rectify}"><span class="glyphicon glyphicon-flash" aria-hidden="true"></span></button>
			</div>
	...
	{if $dnssec.keys}
        {foreach from=$dnssec.keys item=key}
			<tr>
				...
				<td align="center">
					{if $key.active eq '0'}
						<button type="button" onclick="dnssec('activatekey', '{$domain->id}', '{$key.id}');" value='Activate' class="btn btn-success"><span class="glyphicon glyphicon-fire" aria-hidden="true"></span></button>
						<button type="button" onclick="dnssec('deletekey', '{$domain->id}', '{$key.id}');" value="Delete" class="btn btn-danger" ><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button>
						{else}
						<button type="button" onclick="dnssec('deactivatekey', '{$domain->id}', '{$key.id}');" value='Deactivate' class="btn btn-default" ><span class="glyphicon glyphicon-minus" aria-hidden="true"></span></button>
						{/if}
				</td>
